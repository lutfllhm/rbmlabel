...
const StoklabelUsers = () => {
  ...
  const fetchUsers = async () => {
    try {
      setLoading(true)
      const response = await api.get('/stoklabel/users')
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (modalMode === 'create') {
        await api.post('/stoklabel/users', formData)
        toast.success('Pengguna berhasil ditambahkan')
      } else {
        await api.put(`/stoklabel/users/${selectedUser.id}`, formData)
  ...
  const handleDelete = async () => {
    try {
      await api.delete(`/stoklabel/users/${selectedUser.id}`)
  ...
  return (
    <div className="space-y-8">
      <AppPageHero
        ...
        description="Kelola akun untuk modul Stock Label — peran dan status aktif."
      >
        <Button
          app="stoklabel"
          variant="primary"
      ...
      <Card className="p-5 shadow-md shadow-slate-200/25 ring-1 ring-slate-100/80 dark:shadow-black/20 dark:ring-white/[0.04] sm:p-6">
        <div className="space-y-2">
          ...
          <input
            ...
            className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 shadow-sm transition-all placeholder:text-slate-400 hover:border-slate-300 focus:border-stoklabel-500 focus:outline-none focus:ring-2 focus:ring-stoklabel-500/20 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100 dark:placeholder:text-slate-500 dark:hover:border-slate-500 dark:focus:border-stoklabel-400"
          />
        </div>
      </Card>
                ...
                <tr key={user.id} className="transition-colors duration-150 hover:bg-slate-50/90 dark:hover:bg-slate-800/60">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center gap-3">
                      <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-stoklabel-100 text-stoklabel-600 dark:bg-stoklabel-950/50 dark:text-stoklabel-400">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div className="flex items-center justify-end gap-2">
                      <button
                        ...
                        className="rounded-lg p-2 text-stoklabel-600 transition hover:bg-stoklabel-50 dark:text-stoklabel-400 dark:hover:bg-stoklabel-950/30"
                        title="Edit"
                      >
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-start gap-3 rounded-xl border border-slate-200 bg-slate-50/80 p-4 transition-colors hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800/40 dark:hover:bg-slate-800/60">
              <input
                ...
                className="mt-0.5 h-4 w-4 rounded border-slate-300 text-stoklabel-600 transition focus:ring-2 focus:ring-stoklabel-500/20 dark:border-slate-600"
              />
      ...
      >
        ...
        <div className="flex justify-end gap-3">
          ...
          <Button app="stoklabel" variant="danger" onClick={handleDelete}>
            Hapus
          </Button>
        </div>
      </Modal>
